...
db.prototype = {
	...
	cardsetzIndex: function (room, id, zindex) {
		redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cards', id, function (err, res) {
			var card = JSON.parse(res);
			if (card !== null) {
				card.zindex = zindex;
	...
	// joinRoom: function (sid, room, callback) {
	// 	redisClient.get(
	// 		REDIS_PREFIX + '-sid:' + sid + '-rooms', function (err, res) {
	// 			const rooms = res ? JSON.parse(res) : []
	// 			rooms.push(room)
	// 			redisClient.set(REDIS_PREFIX + '-sid:' + sid + '-rooms', JSON.stringify(rooms))
	// 			callback()
	// 		}
	// 	);
	// },
	// getMyRooms: function (sid, callback) {
